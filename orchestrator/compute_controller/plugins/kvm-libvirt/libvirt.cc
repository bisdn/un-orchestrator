...
int Libvirt::cmd_startNF(uint64_t lsiID, string nf_name, string uri_image, unsigned int n_ports, map<unsigned int,pair<string,string> > ipv4PortsRequirements,map<unsigned int,string> ethPortsRequirements)
{
	...
	if (ovsdpdk) {
		...
		for(unsigned int i=1;i<=n_ports;i++) {
			...
			sprintf(port_name, "%sp%ub" "%" PRIu64, nf_name.c_str(), i, lsiID);
...
int Libvirt::cmd_destroy(uint64_t lsiID, string nf_name){
	...
	/*image_name*/
		sprintf(vm_name, "%" PRIu64 "_%s", lsiID, nf_name.c_str());

	/*destroy the VM*/
	if(virDomainDestroy(virDomainLookupByName(conn, vm_name)) != 0){
